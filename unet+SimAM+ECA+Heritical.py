...
def main():
    ...
    ap.add_argument("--outdir", type=str, default="runs_cbct_Unet_SCDA_Heritical_Fusion")
    ...
    #         norm='INSTANCE'
    #     )
    #     # 在 encoder / decoder 的 ResidualUnit 后面统统加 SimAM3D
    #     add_simam_to_unet(net)
    #     return net


    # -------- Models --------
    def make_unet():
        # 自定义 SimAM + ECA + Hierarchical Decoder UNet
        return SimAMHierUNet3D(num_classes=args.num_classes)

    # student / teacher 初始化一次就够了
...
# 让一致性真正起作用
            ...
            with torch.amp.autocast('cuda', enabled=args.amp):
                ...
                w = args.consist_w * sigmoid_rampup(epoch, args.consist_ramp)
                loss_cons_w = w * loss_cons

            # 第二次反传：一致性损失
            scaler.scale(loss_cons_w).backward()
            ...
            # 记录平均 loss（sup + w * cons）
            total_loss = loss_sup + loss_cons_w
