...
GitHub README files cannot implement true JavaScript/CSS tab switching because custom scripts and many interactive styles are restricted. This README therefore uses native Markdown/HTML `<details>` blocks:
...
This keeps both languages on the same page. The English section is expanded by default, and the Chinese section can be expanded without opening another file or requiring JavaScript. A standalone Chinese mirror is also kept in `README.zh-CN.md` for users who prefer a single-language document.
